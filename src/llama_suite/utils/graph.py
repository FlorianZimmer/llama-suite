...
Works with the new repo layout:
- Default: if --summary is missing or doesn't exist, auto-search under ./runs/** for the
  latest summary_all.json (prefers eval runs, falls back to anything under runs).
- Still supports explicit --summary paths.
- Standalone CLI or importable helper `plot_summary(...)`.

Metrics:
  auto | composite | mcq_accuracy | short_em | short_f1 |
  gen_judge_score | latency_s_avg | tokens_prompt_avg | tokens_completion_avg | tokens_total_avg
...
def _is_repo_root(p: Path) -> bool:
    # Heuristics to detect repo root without hard-coding:
    # presence of 'src/llama_suite' OR 'configs' OR 'models' directories.
    return (p / "src" / "llama_suite").exists() or (p / "configs").exists() or (p / "models").exists()
...
def _find_latest_summary(base: Path) -> Optional[Path]:
    """
    Search likely locations for summary_all.json under the new structure.
    Priority:
      1) runs/eval/runs/**/summary_all.json
      2) runs/**/summary_all.json
    Returns the newest (by mtime) if multiple are found.
    """
    candidates: List[Path] = []

    eval_runs = base.glob("runs/eval/runs/**/summary_all.json")
    candidates.extend(eval_runs)

    # Fallback: anything under runs
    other = base.glob("runs/**/summary_all.json")
    candidates.extend(other)

    # Deduplicate while preserving all Paths
    uniq = {}
    for c in candidates:
        try:
            uniq[c.resolve()] = c
    ...
    try:
        ...
        # If stat fails for some, filter to existing
...
def _cli():
    ...
    ap.add_argument("--summary", type=Path, default=None,
                    help="Path to summary_all.json. If omitted or missing, searches ./runs/** for the latest.")
    ap.add_argument("--find-latest", action="store_true",
                    help="Force search under ./runs/** and pick the newest summary_all.json.")
    ...
    # Resolve (and narrow) to a non-optional Path
    summary_path: Optional[Path] = None
    if args.summary is not None:
        summary_path = args.summary if args.summary.is_absolute() else Path.cwd() / args.summary

    needs_search = args.find_latest or summary_path is None or not summary_path.exists()
    if needs_search:
        found = _find_latest_summary(repo_root)
        if not found:
            cwd_fallback = Path.cwd() / "summary_all.json"
            if cwd_fallback.exists():
                found = cwd_fallback
        if not found:
            raise FileNotFoundError(
                "Could not locate summary_all.json. "
                "Provide --summary or ensure there is at least one under ./runs/**"
            )
        summary_path_resolved: Path = found
    else:
        # Static type checkers now know this is a Path
        assert summary_path is not None
        summary_path_resolved: Path = summary_path
