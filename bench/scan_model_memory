...
def main_scan(): # Renamed main for clarity if this were part of a larger suite
    global logger, TEMP_DIR_MANAGER_PATH, project_root_dir # project_root_dir is set by import block

    parser = argparse.ArgumentParser(
        description="LLM Model Memory Scanner.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    # project_root_dir is already defined globally when imports are processed
    ...
    # Example of a more specific default, can be changed or removed
    ...
    parser.add_argument("--health-timeout", type=int, default=DEFAULT_HEALTH_TIMEOUT_S_SCAN, help=f"Override health check timeout (seconds). Default: {DEFAULT_HEALTH_TIMEOUT_S_SCAN}s.")
    ...
    parser.add_argument(
        ...
        help="Keep temporary server log files in a subdirectory ('llm_memscan_persistent_logs') instead of auto-deleting."
    )
    
    args = parser.parse_args()

    # Initialize logger (from config_utils)
    # Optionally, could add a log file specific to this script run
    # log_file_for_script = Path.cwd() / f"scan_model_memory_run_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    # logger = Logger(verbose=args.verbose, log_file_path=log_file_for_script)
    ...
    # Paths relative to where Popen might be called from, or absolute paths
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'exe', 'username']):
        try:
            ...
            if target_port_arg_scan in cmdline_str: is_potential_llama_server = True
            ...
            if is_potential_llama_server:
                # Skip if it's a root/system process NOT using our specific port (heuristic)
                if username and "root" in username.lower() and target_port_arg_scan not in cmdline_str:
                    logger.debug(f"  PID {pid} ('{proc_name_raw}') matched pattern but owned by '{username}'. Skipping kill (not on target port).")
                    continue
                system_daemon_names = ["windowserver", "systemuiserver", "cvmserver", "nfstorageserver"] # macOS/Linux examples
                if proc_name in system_daemon_names and target_port_arg_scan not in cmdline_str:
                    logger.debug(f"  PID {pid} ('{proc_name_raw}') known system daemon. Skipping kill (not on target port).")
                    continue
                
                cmd_display_str = (" ".join(cmdline_list))[:100] if cmdline_list else ""
                logger.warn(f"  Terminating potential lingering llama-server: PID={pid}, Name='{proc_name_raw}', User='{username}', Cmd='{cmd_display_str}'")
                p_obj = psutil.Process(pid); p_obj.terminate()
                try: p_obj.wait(timeout=1)
                except psutil.TimeoutExpired: logger.warn(f"    PID {pid} did not terminate, killing..."); p_obj.kill(); p_obj.wait(timeout=1)
                killed_procs += 1
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess): continue
        except Exception as e_psutil: logger.warn(f"  Error during psutil check for PID {pid if 'pid' in locals() else 'unknown'}: {e_psutil}")

    if killed_procs > 0: logger.success(f"Initial cleanup terminated {killed_procs} process(es).")
    else: logger.info(f"No lingering llama-server processes found (port {STATIC_SERVER_PORT_SCAN} or patterns).")
    ...
    try:
        effective_conf_dict = generate_processed_config(
            ...
            script_dir_for_overrides=project_root_dir, # Pass project_root for override lookup logic
    ...
    health_timeout_final = args.health_timeout if args.health_timeout is not None and args.health_timeout != DEFAULT_HEALTH_TIMEOUT_S_SCAN else \
                           effective_conf_dict.get("healthCheckTimeout", DEFAULT_HEALTH_TIMEOUT_S_SCAN)
    ...
    try:
        ...
        logger.error(f"Could not create output directory {output_file_abs.parent}: {e}"); sys.exit(1)

    script_start_time = time.monotonic()
    temp_dir_path_for_run: Path # Define type hint

    if args.keep_temp_logs:
        # Create a persistent directory for logs
        persistent_log_parent_dir = Path.cwd() / "llm_memscan_persistent_logs"
        persistent_log_parent_dir.mkdir(parents=True, exist_ok=True)
        
        # Optional: Create a timestamped subdirectory for each run to avoid overwriting
        ...
        TEMP_DIR_MANAGER_PATH = temp_dir_path_for_run # Update global for signal handler info
        ...
        # Directly proceed with run_memory_scan using this persistent path
        logger.header("STARTING MEMORY SCAN RUN (Persistent Logs)")
        ...
        run_memory_scan(
            ...
            temp_dir_path=temp_dir_path_for_run, # Pass the persistent path
        ...
        with tempfile.TemporaryDirectory(prefix="llm_memscan_") as temp_dir_name:
            TEMP_DIR_MANAGER_PATH = Path(temp_dir_name) # Store for signal handler info
            ...
            logger.header("STARTING MEMORY SCAN RUN (Auto-deleted Logs)")
            ...
            run_memory_scan(
                ...
                health_poll_s=args.poll_interval,
                model_to_scan_alias=args.model,
                temp_dir_path=temp_dir_path_for_run, # Pass the temporary path
                logger_instance=logger
            )
