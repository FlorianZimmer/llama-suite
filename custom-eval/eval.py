...
(refactored for clarity, safety, and fewer surprises)

Key changes vs. previous version:
- Correct /health probing even when endpoint ends with /v1
- Single, well-placed swap call
- No hidden/unused knobs
- Heavy-model tweaks (retries/backoff/sem) do not leak across models
- More robust MCQ letter parsing
"""
...
def token_f1(pred: str, gold: str) -> float:
    ...
    if not p or not g:
        return 0.0
    # count overlap by unique token type
    common: Dict[str, int] = {}
    for t in set(p):
...
def guard_llama_singleton(keep: int = 1) -> None:
    ...
    if len(procs) <= keep:
        return
    procs_sorted = sorted(procs, key=lambda x: x[0])  # PID ~ start time
...
async def _wait_until_ready(session: aiohttp.ClientSession, endpoint: str, timeout_s: int = 30):
...
async def _enforce_singleton_after_swap(timeout_s: float = 20.0) -> None:
    """After swap/health, ensure only one llama-server remains."""
...
class OpenAICompatClient:
    ...
    async def chat(self, session: aiohttp.ClientSession, model: str, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        if self.cfg.max_tokens is not None: payload["max_tokens"] = self.cfg.max_tokens
        if self.cfg.seed is not None: payload["seed"] = self.cfg.seed
        if self.cfg.stop: payload["stop"] = self.cfg.stop
        ...
        for attempt in range(self.cfg.retries):
            try:
                ...
                # Retry only for network errors or 5xx ChatRequestError
...
def extract_mcq_letter(text: str, n_choices: int) -> Optional[int]:
    """
    Accepts 'A'/'a' optionally followed by punctuation.
    """
...
class Benchmark:
    ...
    async def run_for_model(self, spec: ModelSpec, tasks: List[Task]) -> List[QAResult]:
        # Pre-guard: collapse strays, keep at most one
        ...
        # Swap once per model batch
        self._maybe_swap(spec)

        dsname = spec.display_name or spec.name

        # Adjust concurrency for heavy models (and snapshot originals)
        ...
        try:
            if heavy:
                self.client.sem = asyncio.Semaphore(1)
                ...
                self.client.sem = asyncio.Semaphore(self.client.cfg.max_concurrency)

            async with aiohttp.ClientSession() as session:
                wait_s = self.client.cfg.health_wait_heavy if heavy else self.client.cfg.health_wait

                # health probe (using root /health even if endpoint ends with /v1)
                try:
                    await _wait_until_ready(session, self.client.cfg.endpoint, timeout_s=wait_s)
                except Exception:
                    pass

                # Post-guard: ensure only one llama-server remains
                if self.cfg.llama_guard:
            ...
            self._write_json(self.cfg.out_dir / f"raw_{sanitize(dsname)}.json",
                             [dataclasses.asdict(r) for r in results])
            self._write_json(self.cfg.out_dir / f"summary_{sanitize(dsname)}.json",
                             self._summarize(results, tasks))
            return results
...
def parse_args() -> argparse.Namespace:
    ...
    # Llama guard defaults OFF (you requested --no-llama-guard by default)
    ...
    # Exit cleanup defaults ON (kept as before)
